...
def _llm_generate_faq_question_answer(summary_text: str, full_text: str) -> tuple[str, str]:
    ...
    [핵심 미션]
    - 질문(question): 고객이 검색창에 칠 법한 '현상'을 가장 일상적인 문장으로 작성하십시오. (검색 성능의 핵심)
    - 답변(answer): 마크다운 기호 없이 오직 '줄바꿈'만 사용하여 UI에 맞게 구성하십시오.

    [작성 지침]
    1. 일반화 원칙 (Universal Principle):
    - 상담 기록에 등장하는 '손주, 남편, 친구' 등 특정 대상이나 개인적인 배경 스토리는 모두 삭제하십시오.
    - '고객님'과 같이 특정 대상을 언급하지 마십시오.
    - 특정 지명, 날짜, 개인 정보, 개인의 고유한 상황을 배제하고 모든 고객에게 포괄적으로 적용되는 '보편적 서비스 정보'로 재구성하십시오.
    - 텍스트 외의 특수 기호는 최소화하십시오.
    - 키워드 필드가 따로 없으므로, 문장 안에 핵심적인 현상 단어가 포함되도록 하십시오.

    2. 질문(question): 
    - 고객이 처음 뱉는 불편한 증상 한 문장으로 작성하십시오. 
    - 예: "인터넷이 갑자기 안 돼요", "TV 화면이 검게 나와요", "요금제가 너무 비싸서 낮추고 싶어요"
    - 키워드 필드가 따로 없으므로, 질문 문장 안에 핵심적인 현상 단어가 포함되도록 하십시오.
